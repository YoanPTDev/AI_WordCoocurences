...
class Options():
    ...
    def creer_parseur(self) -> None:
        ...
        self.p.add_argument('-n', type=int, help="Nombre de mots maximal à afficher par cluster")
        self.p.add_argument('-k', type=int, help="Nombre de centroïdes")
        self.p.add_argument('-v', action="store_true", help="Performance times")
        self.p.add_argument('--knn', type=int, help="k nearest neighbors")
    ...
    def parser(self) -> None:
        ...
        if self.e + self.r + self.b + self.c != 1:
            raise Exception("Vous devez choisir une et une seule option parmi -e, -r, -c et -b")
        ...
        if self.e or self.r or self.c:
            ...
            if self.c:
                ...
                if self.n is None or self.n < 1:
                    raise Exception('S.V.P. fournir un nombre maximal de mots à afficher')
                if self.k is None or self.k < 1:
                    raise Exception("S.V.P. fournir un nombre de centroïdes")
                if self.knn is None or self.knn < 1:
                    raise Exception("S.V.P. fournir un nombre de voisins à tester")
